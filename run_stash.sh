...
if [[ ! "$(ls -A /opt/stash/)" ]]; then
    cp -a /opt/.stash.orig/. /opt/stash/. 2>/dev/null || :
    chown -R stash:stash /opt/stash/
fi

chown -R stash:stash /opt/stash-home/
rm -f /opt/stash-home/.stash-home.lock
...
chown stash:stash conf/server.xml
...
download_mysql_driver() {
  ...
  if [ ! -f "$1/$driver-bin.jar" ]; then
    ...
    chown stash:stash $1/$driver-bin.jar
  fi
}
...
sudo -E -u stash /opt/stash/bin/start-stash.sh -fg
